...
test.describe("RAC", () => {
    ...
    test("Configure provider, add devices, attach to application, and launch from user library", async ({
        ...
        const { seed, providerName, appName, deviceA, deviceB } = fixtures.get(testInfo.testId)!;
        ...
        // Every device the user can reach is launchable through the provider, so
        // narrow the launcher to the two this test created.
        await test.step("Filter the launcher to this test's devices", async () => {
            const deviceSearch = launchDialog.getByPlaceholder("Search for a device by name...");

            await deviceSearch.fill(seed);
        ...
        // single protocol auto-launches and closes the modal.
        await test.step("Device list populates on first open", async () => {
            await expect(
                launchDialog.getByRole("cell", { name: deviceA }),
                `Device ${deviceA} is visible in the launcher`,
            ).toBeVisible();

            await expect(
                launchDialog.getByRole("cell", { name: deviceB }),
                `Device ${deviceB} is visible in the launcher`,
            ).toBeVisible();
        });

        await test.step("Each device offers the protocol it is reached with", async () => {
            await expect(
                launchDialog.getByRole("button", { name: "VNC" }).first(),
                "Device can be launched with VNC",
            ).toBeVisible();
        });
